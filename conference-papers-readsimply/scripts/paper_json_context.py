#!/usr/bin/env python3
"""Prepare paper JSON context for the conference-papers-readsimply skill.

Source: adapted from conference-papers-read2md/scripts/paper_json_context.py.
Changes: targets abstract-only notes, prefers arXiv HTML abstract metadata,
omits assets, and emits `simple_note_path` with the `{MethodName}-simple.md`
convention.
"""
...
def abstract_source_candidates(
    paper: dict[str, Any],
    paper_path: Path,
    site: Path,
    arxiv_id: str,
    ...
    if arxiv_html:
        candidates.append({"type": "arxiv_html_abstract", "url": arxiv_html})
    if clean_text(paper.get("abstract")):
        candidates.append({"type": "json_abstract", "url": path_relative_to(paper_path, site)})
    if arxiv_url:
        candidates.append({"type": "arxiv_abs", "url": arxiv_url})
    elif arxiv_id:
        candidates.append({"type": "arxiv_abs", "url": arxiv_abs_url(arxiv_id)})
    if not arxiv_id and query:
        candidates.append({"type": "arxiv_search", "url": query})
    for key, source_type in (("detail_url", "conference_detail"), ("url", "conference_page")):
...
def load_paper_context(
    ...
    method_name = suggest_method_name(title)
    simple_note_path = inbox_dir / f"{method_name}-simple.md"
    venue = clean_text(paper.get("conference") or paper.get("venue") or ("arXiv" if arxiv_id else ""))
    candidates = abstract_source_candidates(paper, paper_path, site, arxiv_id, arxiv_url, html_url, query)
    ...
    return {
        ...
        "abstract_available": bool(clean_text(paper.get("abstract")) or candidates),
        "arxiv_id": arxiv_id,
        "arxiv_url": arxiv_url,
        "arxiv_html_url": html_url,
        "pdf_url": pdf_url,
        "project_url": clean_text(paper.get("project_url")),
        "abstract_source_candidates": candidates,
        "primary_source": candidates[0] if candidates else {"type": "missing", "url": ""},
        "arxiv_search_query": query,
        "suggested_method_name": method_name,
        "paper_json": path_relative_to(paper_path, site),
        "output_dir": path_relative_to(inbox_dir, site),
        "simple_note_path": path_relative_to(simple_note_path, site),
    }


def main() -> int:
    parser = argparse.ArgumentParser(description="Read conference paper JSON and print readsimply context.")
