...
conference-papers-readsimply/   # 从 paper JSON/arXiv HTML abstract 批量生成 Obsidian 简读 Markdown
conference-papers-site/         # 从已有 data/ 构建静态网站入口
conference-papers-maintain/     # 增量维护、去重、补 metadata、修内链入口
```

除 `conference-papers-read2md` 和 `conference-papers-readsimply` 是独立 Markdown 导出 skill 外，其余 wrapper skill 都很薄，真正实现集中在 `conference-papers/scripts/`。这样可以避免多份脚本漂移。
...
cp -R conference-papers-readsimply ~/.codex/skills/
...
安装后重启 Codex。七个目录建议保持同级；其中 wrapper skills 会调用：
...
生成 Obsidian 兼容简读 Markdown 到 inbox（优先读 arXiv HTML 摘要与元信息，不参与建站）：

```text
Use $conference-papers-readsimply 批量简读 data/papers/*.json，生成 Obsidian md 到 data/_inbox。
```
...
如果只想快速基于 arXiv HTML abstract 生成简读笔记，可以运行：

```text
Use $conference-papers-readsimply 简读 data/papers/xxx.json
Use $conference-papers-readsimply 批量简读 data/papers/*.json
Use $conference-papers-readsimply 批量简读 data/papers/
```

`conference-papers-read2md` 会读取 `data/papers/*.json` 中的标题、作者、arXiv/PDF/项目主页等信息，生成：
...
`conference-papers-readsimply` 优先通过 arXiv HTML 只读取摘要和页面元信息，并用 paper JSON 补会议、类别、项目链接等字段；如果 arXiv HTML 不可用，再回退到 JSON `abstract`、arXiv abs/API 或会议详情页摘要。默认生成：

```text
data/_inbox/{paper-id}/{MethodName}-simple.md
```

它不会继续阅读 Introduction/Method/Experiment/Appendix，不下载 PDF，不创建 `assets/`，也不修改 `data/papers/*.json` 或 `paper["note"]`。
...
python3 ~/.codex/skills/conference-papers-readsimply/scripts/paper_json_context.py \
  data/papers/fast-efficient-action-tokenization-for-vision-language-action-models.json \
  --site .
```
...
```text
"simple_note_path": "data/_inbox/fast-efficient-action-tokenization-for-vision-language-action-models/FAST-simple.md"
"primary_source": {
  "type": "arxiv_html_abstract"
}
```

如果输出里有 `arxiv_html_url`，可以单独检查 arXiv HTML 摘要和元信息解析：

```bash
python3 ~/.codex/skills/conference-papers-readsimply/scripts/arxiv_html_metadata.py \
  "$(python3 ~/.codex/skills/conference-papers-readsimply/scripts/paper_json_context.py \
    data/papers/fast-efficient-action-tokenization-for-vision-language-action-models.json \
    --site . | python3 -c 'import json,sys; print(json.load(sys.stdin)["arxiv_html_url"])')"
```

批量展开可以运行：

```bash
python3 ~/.codex/skills/conference-papers-readsimply/scripts/batch_paper_json_context.py \
  'data/papers/*.json' \
  --site .
```
...
完成后应看到每篇论文对应的 `{MethodName}-simple.md`，且不会创建 `assets/`。
...
python3 /Users/barry/.codex/skills/.system/skill-creator/scripts/quick_validate.py conference-papers-read2md
python3 /Users/barry/.codex/skills/.system/skill-creator/scripts/quick_validate.py conference-papers-readsimply
