...
2. Read the helper JSON output and create the inbox directory:
   ...
   - `assets_dir`: normally `data/_inbox/{paper-id}/assets`
...
6. Localize images into the inbox.
   - During drafting, image links may be external Markdown links.
   - After saving the note, run:
...
```
   ...
   - If an image cannot be downloaded, use the paper PDF/project page to obtain the figure manually and place it in `assets/`.
...
- Preserve the dailypaper sections: metadata, one-sentence summary, core contributions, background, method details, key formulas, figures/tables, experiments, critical thinking, related notes, and quick card.
...
- `scripts/localize_note_images.py`: download note images into inbox `assets/` and rewrite links.
